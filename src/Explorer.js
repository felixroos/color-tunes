...
import { randomElement } from 'jazzband/lib/util';
...
export default class Explorer extends React.Component {
  ...
  pressedPianoKey(key) {
    ...
    this.pianist.instrument.playNotes([note], {
      duration: 1000,
      pulse: this.pulse
    });
  }
  ...
  fifthDown() {
    if (!this.state.fixedChroma) {
      this.setState(
        newTonicState(Distance.trFifths(this.state.tonic, -1), this.state)
      );
    } else {
      this.setState(
        newTonicState(
          this.getStep(this.state.fixedChroma, this.state.tonic, 3),
          this.state
        )
      );
    }
  ...
  nextStep(props, distance = 1) {
    ...
    const intervals = parallel
      .map((p, i) => ({
        ...
        root: p.root,
        distance:
          (Interval.semitones(Distance.interval(props.tonic, p.root)) + 12) % 12
      }))
    ...
    this.setState(
      Object.assign(
        { [props.chord ? 'chord' : 'scale']: next.symbol },
        newTonicState(next.root, this.state)
      )
    );
    this.autoplay();
  }
  ...
  nextSibling(props, distance = 1) {
    ...
    const intervals = parallel
      .map((p, i) => ({
        i,
        root: p.root,
        distance: (Distance.fifths(p.root, props.tonic) + 12) % 12
      }))
      .filter(d => d.distance >= distance)
      .sort((a, b) => (a.distance < b.distance ? -1 : 1));
    if (!intervals.length) {
  ...
  randomTonic(maxFifthDistance) {
    ...
    if (!maxFifthDistance) {
      ...
      const fifthDistance =
        (Math.random() > 0.5 ? -1 : 1) *
        Math.ceil(Math.random() * maxFifthDistance);
  ...
  randomChordOrScale(keepTonic = false, type) {
    const isChord =
      type === 'chord' ? true : type === 'scale' ? false : Math.random() > 0.5;
  ...
  playPiano(
    notes,
    duration = this.state.arpeggioInterval * 1000,
    tonic = this.state.tonic
  ) {
    if (!this.state.arpeggiate) {
      duration = 3000;
    }
    this.pianist.playNotes(notes, {
      duration,
      tonic,
      pulse: this.pulse,
      interval: this.state.arpeggiate ? this.state.arpeggioInterval : 0
    });
  }
  ...
  toggleIntelligentVoicings() {
    this.pianist.props.intelligentVoicings = !this.pianist.props
      .intelligentVoicings;
    this.setState({
      intelligentVoicings: this.pianist.props.intelligentVoicings
    });
  }

  playDrone(notes) {
    this.stopDrone(notes);
    const droneVoices = this.drone.playNotes(notes, {
      endless: true,
      decay: 0,
      attack: this.state.fade,
      deadline: 0,
      sustain: 1,
      gain: 0.1
    });
  ...
  render() {
    ...
    const bgColor = pitchColor(
      props.tonic,
      colorConfig.saturationDefault,
      colorConfig.brightnessDefault
    );
    const color = pitchColor(
      props.tonic,
      colorConfig.saturationActive,
      colorConfig.brightnessActive
    );
    const highlight = pitchColor(
      props.tonic,
      colorConfig.saturationActive,
      colorConfig.brightnessActive
    );
    ...
    const circles = ['fourths', 'fifths', 'chromatics'].map((circle, index) => {
      return (
        <li
          key={index}
          className={this.state.circle === circle ? 'active' : ''}
          onClick={() => this.setState({ circle })}
        >
          {circle}
        </li>
      );
    ...
    const groups = groupNames().map((group, index) => {
      return (
        <li
          key={index}
          className={this.state.group === group ? 'active' : ''}
    ...
    const material = (
      <Material
        key="material"
        props={props}
        onClick={data => this.useData(data)}
      />
    );
      ...
      <div className="explorer">
        ...
        <div className="symbols">
          <div className="explorer-content">
            ...
            {/* <h5>Steps</h5>
            <Steps steps={props.nodes} notes={props.notes} /> */}
            ...
            <ul className="scroll">
              ...
              <li
                className={this.state.showSteps ? 'active' : ''}
                onClick={() => {
              ...
              <li
                onClick={() => this.invert(props.notes)}
                className={this.state.invert > 0 ? 'active' : ''}
              >
              ...
              <li
                onClick={() => this.setState({ order: null, invert: 0 })}
                className={
                  this.state.order || this.state.invert > 0 ? 'parallel' : ''
                }
              >
                clear
              </li>
            ...
            <ul className="scroll">
              <li
                onClick={() =>
                  this.setState({
                    fixedChroma: this.state.fixedChroma ? null : props.chroma,
                    fixedLabel: props.label
                  })
                }
                className={this.state.fixedChroma ? 'active' : ''}
              >
                {!this.state.fixedChroma ? 'focus' : 'blur'}{' '}
                {this.state.fixedChroma ? this.state.fixedLabel : props.label}
              </li>
              <li
                onClick={() => this.nextSibling(props)}
                className={this.state.fixedChroma ? 'active' : ''}
              >
                +harmonic sibling
              </li>
              ...
              <li
                onClick={() => this.harmonicSibling(props)}
                className={this.state.fixedChroma ? 'active' : ''}
              >
                % harmonic sibling
              </li>
            ...
            <ul className="scroll">
              <li
                onClick={() =>
                  this.nextRoot(this.state.fixedChroma || props.chroma)
                }
                className={this.state.fixedChroma ? 'parallel' : ''}
              >
                +step
              </li>
              <li
                onClick={() => this.fifthDown()}
                className={this.state.fixedChroma ? 'parallel' : ''}
              >
                -fifth
              </li>
              ...
              <li
                onClick={() =>
                  this.randomChordOrScale(this.state.fixedTonic, 'chord')
                }
              >
                % chord
              </li>
              <li
                onClick={() =>
                  this.randomChordOrScale(this.state.fixedTonic, 'scale')
                }
              >
                % scale
              </li>
              <li
                className={this.state.fixedTonic ? 'active' : ''}
                onClick={() =>
                  this.setState({ fixedTonic: !this.state.fixedTonic })
                }
              >
                fixedTonic
              </li>
            ...
            <h5>Drone / Piano</h5>
            <ul className="scroll">
              <li onClick={() => this.playPiano(props.scorenotes)}>
                play piano
              </li>
              <li
                className={this.state.arpeggiate ? 'active' : ''}
                onClick={() =>
                  this.setState({ arpeggiate: !this.state.arpeggiate })
                }
              >
                arpeggiate piano
              </li>
              <li
                className={this.state.autoplay ? 'active' : ''}
                onClick={() =>
              ...
              <li
                className={this.state.droneVoices ? 'active' : ''}
                onClick={() => this.toggleDrone(props.scorenotes)}
              >
                play drone
              </li>
              <li
                className={this.state.intelligentVoicings ? 'active' : ''}
                onClick={() => this.toggleIntelligentVoicings()}
              >
                intelligent voicings
              </li>
          ...
          <div className="aside">
            {/* <Circle
              size="250"
              width="100%"
              set={props.order.map(o => props.nodes.indexOf(props.labels[o]))}
              onClick={note => this.newTonic(note)}
              origin={props.labels[props.order[0]]}
              nodes={props.nodes}
            /> */}


            <div className="circle">{!this.state.hideCircle ? circle : ''}</div>
            <div className="scroll-content">
              <ul className="scroll">{groups}</ul>
              {views}
              <h2>Settings</h2>
              <h5>Views</h5>
              <ul>
                <li
                  className={this.state.flip ? 'active' : ''}
                  onClick={() => this.setState({ flip: !this.state.flip })}
                >
                  Flip
                </li>
                <li
                  className={!this.state.hidePiano ? 'active' : ''}
                  onClick={() =>
                    this.setState({ hidePiano: !this.state.hidePiano })
                  }
                >
                  Piano
                </li>
                <li
                  className={!this.state.hideScore ? 'active' : ''}
                  onClick={() =>
                    this.setState({ hideScore: !this.state.hideScore })
                  }
                >
                  Score
                </li>
                <li
                  className={!this.state.hideCircle ? 'active' : ''}
                  onClick={() =>
                    this.setState({ hideCircle: !this.state.hideCircle })
                  }
                >
                  Circle
                </li>
              ...
              <ul className="scroll">
                {circles}
                <li
                  className={this.state.ordered ? 'active' : ''}
                  onClick={() =>
              ...
              <h2>Help</h2>
              This tool visualizes the connection between musical chords and
              scales. The colors have the following meanings:
              <ul>
              ...
              <p>
                <strong>abstraction</strong> = less notes
              </p>
              <p>
                <strong>extension</strong> = more notes
              </p>
              You can change the current root by clicking the desired note in
              the circle.
              <br />
              In the Material view, all listed chords and scales are
              abstractions or extensions but only the ones with the current
              selected root are highlighted.
              <br />
              You can filter the displayed chords and scales to focus on
              specific groups of symbols.
              <br />
              <strong>Pro Tip: </strong> You scroll/swipe the listings
              horizontally!
            </div>
          </div>
        </div>
