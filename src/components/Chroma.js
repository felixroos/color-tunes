...
export function sortByChordLength() {
  return (a, b) =>
    Chord.notes('C' + a).length < Chord.notes('C' + b).length ? -1 : 1;
}

export function sortByScaleLength() {
  return (a, b) =>
    Scale.notes('C', a).length < Scale.notes('C', b).length ? -1 : 1;
}
...
export function sortByDistanceToTonic(tonic) {
  return (a, b) =>
    Distance.semitones(tonic, a.root) < Distance.semitones(tonic, b.root)
      ? -1
      : 1;
}
export function sortByFifthDistanceToTonic(tonic) {
  return (a, b) =>
    Distance.fifths(tonic, a.root) < Distance.fifths(tonic, b.root) ? -1 : 1;
}
...
export function chromaParallels(chroma, group) {
  return {
    ...
    /* .filter(filterScaleLength(7))
                .sort(sortByScaleLength()) */ chords: chordNames(
      group
    )
      .map(type => {
        return {
...
export function getSupersets(type, isScale, group) {
  const isSuperset = PcSet.isSupersetOf(
    (isScale ? Scale : Chord).intervals(type)
  );
  return {
    scales: isScale
      ? Scale.supersets(type)
      : scaleNames(group).filter(scale => isSuperset(Scale.intervals(scale))),
    chords: !isScale
      ? Chord.supersets(type)
      : chordNames(group).filter(chord => isSuperset(Chord.intervals(chord)))
  };
}
...
export function getSubsets(type, isScale, group) {
  ...
  return {
    scales: isScale
      ? Scale.subsets(type)
      : scaleNames(group).filter(scale => isSubset(Scale.intervals(scale))),
    chords: !isScale
      ? Chord.subsets(type)
      : chordNames(group).filter(chord => isSubset(Chord.intervals(chord)))
  };
}
...
export function parallelSymbols(type, parallel, props) {
  return props.parallels[type + 's'].reduce((items, item, index) => {
    return items
      .concat(item[parallel].map(root => ({ root, symbol: item.symbol })))
...
export function getSmallestInterval(pc1, pc2) {
  ...
  return up > down
    ? '-' + Interval.fromSemitones(down)
    : Interval.fromSemitones(up);
}
...
export function getEnvelope(scorenotes) {
  ...
  return scorenotes
    ...
    .sort()
    .map(midi => scorenotes.find(n => Note.midi(n) === midi))
...
export function newTonicState(tonic, state) {
  ...
  let newTonic = Note.simplify(
    Distance.transpose(state.tonic + state.octave, smallestInterval)
  );
...
export function getProps(state) {
  ...
  scorenotes = transposeToRange(scorenotes, ['A3', 'C6']);
  ...
  if (state.invert) {
    ...
    scorenotes = scorenotes.map((note, index) =>
      index < state.invert ? transpose(note, Interval.fromSemitones(12)) : note
    );
...
export function symbolClasses(type, symbol, props, differentRoot) {
  differentRoot = differentRoot
    ? Note.midi(props.tonic + '2') !== Note.midi(differentRoot + '2')
    : null;
